...
class HHParser:
    ...
    def categorize_vacancy(self, vacancy):
        ...
        text = f" {title} {desc} {skills} "  # Пробелы для точного поиска
        
        # Сортируем категории по приоритету (от высшего к низшему)
        ...
        for category_name, category_data in sorted_categories:
            ...
            # Для некоторых категорий проверяем исключения
        ...
        for keyword in KEYWORDS:
            ...
            while page < total_pages:
                ...
                try:
                    ...
                    total_pages = min(data.get('pages', 1), 3)  # Максимум 2 страницы
                    ...
                    for item in data.get('items', []):
                        ...
                        if datetime.now(published.tzinfo) - published > timedelta(days=15):
                            continue
            ...
            while page < total_pages:
                ...
                try:
                    ...
                    for item in data.get('items', []):
                        # Пропускаем старые вакансии
                        published = datetime.fromisoformat(item['published_at'].replace('Z', '+00:00'))
                        if datetime.now(published.tzinfo) - published > timedelta(days=2):
                            continue
